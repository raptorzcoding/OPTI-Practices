...
void GRAFO::PDM()
{
    deque<unsigned> dcola; // Cola de nodos
    vector<int> d; // Vector de distancias
    vector<unsigned> pred; // Vector de predecesores
    vector<bool> Encola; // Vector de nodos en la cola
    unsigned s; // Nodo inicial introducido por el usuario
    int Cmin; // Cmin para calcular los circuitos de coste negativo
    ...
    cin >> s;
    s--; // Le restamos uno para que corresponda al índice del vector
    ...
    dcola.push_back(s); Encola[s] = true;
      while (!dcola.empty())
      {
        unsigned k = dcola.front(); // Sacamos el primer nodo, k, de la dcola
        dcola.pop_front(); // Borramos el nodo k de la dcola
        Encola[k] = false; // Actualizamos Encola

        for (int p = 0; p < LS[k].size(); p++)
        {
          unsigned j = LS[k][p].j; // Nodo j
          int ckj = LS[k][p].c; // Peso del camino entre k y j

          if (d[j] > d[k] + ckj) // Si la distancia de j es mayor que la distancia de k + el peso del camino entre k y j
          {
            if (pred[j] == UERROR) // Si el nodo j nunca ha estado en la cola
            {
              dcola.push_back(j); // Insertamos j al final de la cola
            }
            else // Si j ya ha estado en la cola
            {
              if (!Encola[j]) // Si j no está en la cola
              {
                dcola.push_front(j); // Insertamos j al principio de la cola
              }
            }

            d[j] = d[k] + ckj; // Actualizamos la distancia de j
            pred[j] = k; // Actualizamos el predecesor de j
            Encola[j] = true; // Actualizamos Encola
          }
        }
      }

      // Aquí iria la comprobacion de circuitos de coste negativo. (No me salió)
      ...
      // En esta parte del código, mostramos los caminos mínimos para cada nodo si los hay, y siempre y cuando no haya circuitos de coste negativo.
      for (unsigned i = 0; i < n; i++)
      {
        if (d[i] != maxint)
        {
          cout << "Camino minimo desde " << s + 1 << " hasta " << i + 1 << ": ";
